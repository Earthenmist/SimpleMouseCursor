...
Core runtime for cursor positioning, rings, trail, reticle, and events.
]]--

SimpleMouseCursor = SimpleMouseCursor or {}
local SMC = SimpleMouseCursor

-- Cache frequently used globals for update loops.
...
-- Cache rotation colors for the update loop.
...
function SMC:UpdateMainRingRotation()
    if not (SMC_Settings and SMC_Settings.enableMainRingRotation) then
        return
    end
    ...
    local speed = tonumber(SMC_Settings.mainRingRotSpeed) or 1.0
    if speed <= 0 then speed = 1.0 end

    local t = GetTime()
    local angle = (t * -2 * speed) % (2 * math.pi)

    if SMC_CursorFrame.MainRing.SetRotation then
...
function SMC:OnUpdate(elapsed)
    ...
    elseif (not shiftDown and SMC.lastShiftState and SMC_Settings.shiftAction == "Show Crosshair") or
        ...
        -- Fade after releasing "Show Crosshair".
...
function SMC:StartGCDAnimation(startTime, duration)
    ...
    -- Avoid arithmetic on secret cooldown values.
    ...
    local function GCDUpdate(self, elapsed)
        ...
        local remaining = duration - elapsedCD
...
function SMC:UpdateHealthRing()
    ...
    -- Avoid arithmetic on secret health values.
    ...
    if not healthPercent then
        -- Hide if health cannot be computed safely.
...
function SMC:UpdatePowerRing()
    ...
    if issecretvalue and issecretvalue(currentPower) then
        -- Prefer percent APIs for secret values.
...
function SMC:GCDCastHandler(self, event, unit, spellName, spellId)
    ...
    -- Let the cooldown frame handle secret durations.
...
function SMC:UpdateVisibility(forceState)
    ...
    if SMC_Settings.showOnlyInCombat then
        ...
        -- Hide by default when "Show Rings" is used as push-to-show.
...
function SMC:SetupUI()
    ...
    -- Use the configured frame strata.
    ...
    -- Horizontal lines use height for thickness.
