...
Settings defaults, option lists, panel setup, and ApplySettings().
]]--
...
-- Opacity is handled by dedicated sliders.
...
pulseSpeedSlider:SetScript("OnValueChanged", function(self, value)
    ...
    local function RefreshRotationSwatches()
        -- Ensure rotation colors have alpha values.
...
rotColor2Button:SetBackdrop({
    ...
    local function UpdateRotationUIState()
        -- Rotation overrides class color.
        ...
        -- Dim rotation labels when disabled.
...
-- Apply SavedVariables to live frames.
